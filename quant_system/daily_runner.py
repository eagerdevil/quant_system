...
def compute_portfolio_summary(portfolio, scores):
    ...
    if has_flows:
        ...
        total_invested = round(total_deposits - total_withdrawals, 2)  # 累计净投入(流水推算)
...
def format_report(plan, scores, timing, portfolio, all_data=None, stock_scores=None, port_summary=None):
    ...
    if ps.get("total_pnl_all") is not None:
        ...
        if ps.get("total_invested_note"):
            detail = f" | 累计净投入(用户基准): {ps.get('total_invested', 0):,.2f}元"
        else:
            detail = f" | 累计净投入: {ps.get('total_invested', 0):,.2f}元" \
                     f" (入金{ps.get('total_deposits', 0):,.2f} - 出金{ps.get('total_withdrawals', 0):,.2f})"
        lines.append(f"  总盈亏: {ps.get('total_pnl_all', 0):+.2f}元 ({ps.get('total_pnl_all_pct', 0):+.2f}%)" + detail)
